...
void NoData_Setup0(void)
{
  ...
  {
    ControlState = STALLED;
    Debug_Print("[EP0] STALL nd req=%02X val=%04X idx=%04X\r\n",
                pInformation->USBbRequest, pInformation->USBwValue, pInformation->USBwIndex);
    goto exit_NoData_Setup0;
  }
...
void Data_Setup0(void)
{
  ...
  {
    Debug_Print("[EP0] STALL d req=%02X val=%04X idx=%04X\r\n",
                pInformation->USBbRequest, pInformation->USBwValue, pInformation->USBwIndex);
 ...
 */
volatile uint32_t setupCount = 0;
/* EP0 request log buffer - dumped from main loop since Debug_Print
 * doesn't work reliably from ISR context */
volatile uint8_t ep0log[8][6];  /* 8 entries: req, val_hi, val_lo, idx_hi, idx_lo, len */
volatile uint8_t ep0logIdx = 0;
uint8_t Setup0_Process(void)
{
  ...
  setupCount++;
  {
    volatile uint8_t *e = ep0log[ep0logIdx & 7];
    e[0] = pInformation->USBbRequest;
    e[1] = (uint8_t)(pInformation->USBwValue >> 8);
    e[2] = (uint8_t)(pInformation->USBwValue & 0xFF);
    e[3] = (uint8_t)(pInformation->USBwIndex >> 8);
    e[4] = (uint8_t)(pInformation->USBwIndex & 0xFF);
    e[5] = (uint8_t)(pInformation->USBwLength & 0xFF);
    ep0logIdx++;
  }
  pInformation->ControlState = SETTING_UP;

  Debug_Print("[EP0] req=%02X val=%04X(b0=%02X b1=%02X) idx=%04X(b0=%02X b1=%02X) len=%04X\r\n",
              pInformation->USBbRequest, pInformation->USBwValue,
              pInformation->USBwValue0, pInformation->USBwValue1,
              pInformation->USBwIndex,
              pInformation->USBwIndex0, pInformation->USBwIndex1,

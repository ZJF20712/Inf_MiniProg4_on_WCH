...
static uint32_t DAP_SWJ_Clock(const uint8_t *request, uint8_t *response) {
  ...
  if (clock >= MAX_SWJ_CLOCK(DELAY_FAST_CYCLES)) {
    ...
    /* clamp to the rate the bit-bang engine sustains robustly on this
     * target (~1 MHz): faster requests fault the CCG5 test controller
     * randomly in long sequences */
    if (DAP_Data.clock_delay < 24U) DAP_Data.clock_delay = 24U;
  }
...
void DAP_Setup(void) {
  ...
  DAP_Data.transfer.idle_cycles = 4U;   /* CCG5 test controller needs idle
                                           separators between transfers */
